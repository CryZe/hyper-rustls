    ...
    T::Future: 'static,
{
    ...
    fn connect(&self, dst: connect::Destination) -> Self::Future {
        ...
        if !is_https {
            let fut = connecting.map_ok(|(tcp, conn)| (MaybeHttpsStream::Http(tcp), conn));
            HttpsConnecting(Box::new(fut))
        } else {
            let cfg = self.tls_config.clone();
            let connector = TlsConnector::from(cfg);
            let fut = connecting
                .map_ok(move |(tcp, conn)| (tcp, conn, hostname))
                .and_then(|(tcp, conn, hostname)| {
                    ready(match DNSNameRef::try_from_ascii_str(&hostname) {
                ...
                .and_then(move |(tcp, conn, dnsname)| {
                    connector
                        ...
                        .and_then(|tls| {
                            ...
                            ready(Ok((MaybeHttpsStream::Https(tls), connected)))
                        })
...
pub struct HttpsConnecting<T>(
    Box<
        dyn Future<Output = Result<(MaybeHttpsStream<T>, connect::Connected), io::Error>>
            + Send
            + Unpin,
    >,
);
...
impl<T> Future for HttpsConnecting<T> {
    type Output = Result<(MaybeHttpsStream<T>, connect::Connected), io::Error>;
